...
//        // ---- END DEBUG ----
  ...
  def fromMetadataFast(metadata: String): Either[TilingError, TilingDCEL] =
    try
      ...
      val vertices = vertexAttrs.map: attrs =>
        Vertex(VertexId(attrs("id").toInt), BigPoint(BigDecimal(attrs("x")), BigDecimal(attrs("y"))))

      val vertexMap = vertices.map(v => v.id -> v).toMap

      val heIdAndAttrs = halfEdgeAttrs0.map: attrs =>
        attrs("id").toInt -> attrs

      val heAllocated = heIdAndAttrs.map: (id, attrs) =>
        id -> HalfEdge(vertexMap(VertexId(attrs("origin").toInt)))

      val halfEdgeMap = heAllocated.toMap
      val halfEdges   = heAllocated.sortBy(_._1).map(_._2)

      val faces   = faceAttrs.map(attrs => Face(FaceId(attrs("id").toInt)))
      val faceMap = faces.map(f => f.id -> f).toMap

      vertexAttrs.zip(vertices).foreach: (attrs, vertex) =>
        attrs.get("leaving").foreach: id =>
          vertex.leaving = Some(halfEdgeMap(id.toInt))

      heIdAndAttrs.foreach: (id, attrs) =>
        val he = halfEdgeMap(id)
        he.twin = Some(halfEdgeMap(attrs("twin").toInt))
        he.next = Some(halfEdgeMap(attrs("next").toInt))
        he.prev = Some(halfEdgeMap(attrs("prev").toInt))
        he.incidentFace = Some(faceMap(FaceId(attrs("face").toInt)))
        he.angle = Some(AngleDegree(Rational(attrs("angle"))))

      faceAttrs.zip(faces).foreach: (attrs, f) =>
        attrs.get("outer-component").foreach: id =>
          f.outerComponent = Some(halfEdgeMap(id.toInt))
        attrs.get("inner-components").filter(_.nonEmpty).foreach: idsStr =>
          f.innerComponents = idsStr.split(',').map(id => Some(halfEdgeMap(id.trim.toInt))).toList
      ...
      case e: Throwable => Left(ValidationError(s"Metadata parsing failed: ${e.getMessage}"))
