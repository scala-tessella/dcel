...
object TilingSVG:
  ...
  extension (tiling: TilingDCEL)
    ...
    /** Serializes the complete structure of a [[TilingDCEL]] into XML metadata, which can be embedded within an SVG. This
      * metadata includes all vertices, half-edges, and faces, along with their properties and relationships,
      * ensuring that the [[TilingDCEL]] can be fully reconstructed later.
      */
  ...
  /** Deserializes the XML metadata that includes all vertices, half-edges, and faces,
   *  along with their properties and relationships,
   *  and fully reconstructs the complete structure of a [[TilingDCEL]].
   */
  def fromMetadata(metadata: String): Either[TilingError, TilingDCEL] =
    ...
    def attrAs[T](node: Node, attr: String, f: String => T, typeName: String): Either[ValidationError, T] =
      for {
        ...
        res <- Try(f(str)).toEither.left.map(e => ValidationError(s"Invalid $typeName in ${node.label} attribute '$attr': ${e.getMessage}"))
      } yield res

    for {
      xmlRoot <- Try(XML.loadString(metadata)).toEither.left.map(e => ValidationError(s"Invalid XML: ${e.getMessage}"))
      ...
      halfEdges <- halfEdgeNodes.map { heNode =>
        for {
          ...
          origin <- vertexMap.get(VertexId(originId)).toRight(NotFoundError("Vertex for half-edge origin", originId))
        } yield HalfEdge(origin)
      }.sequence
      halfEdgeMap = halfEdges.zipWithIndex.map { case (he, i) => i -> he }.toMap
      ...
      _ <- vertexNodes.zip(vertices).map { case (vNode, vertex) =>
        vNode.attribute("leaving").map(_.text).traverse { leavingIdStr =>
          for {
            leavingId <- Try(leavingIdStr.toInt).toEither.left.map(_ => ValidationError(s"Invalid leaving ID: $leavingIdStr"))
            leavingEdge <- halfEdgeMap.get(leavingId).toRight(NotFoundError("Leaving edge", leavingId.toString))
      ...
      _ <- halfEdgeNodes.zip(halfEdges).map { case (heNode, he) =>
        for {
          ...
          angle = AngleDegree(Rational(angleStr))//.toRight(ValidationError(s"Invalid angle format: $angleStr"))
          _ = he.angle = Some(angle)
        } yield ()
      }.sequence

      _ <- faceNodes.zip(faces).map { case (fNode, f) =>
        val z = fNode.attribute("outer-component").map(_.text).traverse { ocIdStr =>
          for
            ocId <- Try(ocIdStr.toInt).toEither.left.map(_ => ValidationError(s"Invalid outer-component ID: $ocIdStr"))
            ocEdge <- halfEdgeMap.get(ocId).toRight(NotFoundError("Outer component edge", ocId.toString))
          yield f.outerComponent = Some(ocEdge)
        }
        fNode.attribute("inner-components").map(_.text).filter(_.nonEmpty).traverse { icIdsStr =>
          val idsEither = icIdsStr.split(',').toList.map(idStr =>
            Try(idStr.trim.toInt).toEither.left.map(_ => ValidationError(s"Invalid inner-component ID: $idStr"))
          ).sequence
          for {
            ids <- idsEither
            icEdges <- ids.map(id => halfEdgeMap.get(id).toRight(NotFoundError("Inner component edge", id.toString))).sequence
          } yield f.innerComponents = icEdges.map(Some(_))
        }
      }.sequence

      outerFace <- faceMap.get(FaceId.outerId).toRight(ValidationError("Outer face (ID 0) not found in metadata"))
