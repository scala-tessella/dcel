...
/** Opaque integer identifier for a [[Face]]. Prefixed string form is `F<n>` (e.g. `F7`) via the
  * [[Prefixable]] mixin on the companion. Two reserved values: [[FaceId.outerId]] (`F0`, the unbounded outer
  * face) and [[FaceId.firstInnerId]] (`F1`, the first inner face produced by constructors).
  */
...
object FaceId extends Prefixable:
  ...
  /** Parses a prefixed string (e.g. `"F7"`) back into a [[FaceId]]. Returns [[ValidationError]] if the input
    * is null, lacks the `F` prefix, or has no valid numeric tail.
    */
