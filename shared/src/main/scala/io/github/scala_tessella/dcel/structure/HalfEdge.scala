...
final class HalfEdge(
  ...
  /** Computes the next half-edge in a sequence based on the twin relationship.
    *
    * This method takes a HalfEdge as input, retrieves its twin (if available), and then attempts to extract
    * the next HalfEdge in the sequence from that twin.
    *
    * @return
    *   An Option containing the next HalfEdge if it exists, or None if the twin or the next HalfEdge is not
    *   defined.
    */
  def maybeNextOfTwin: HalfEdge => Option[HalfEdge] =
    halfEdge =>
      halfEdge.twin.flatMap: maybeHalfEdge =>
        maybeHalfEdge.next

  private[dcel] def vertexTraversalUnsafe[T](f: HalfEdge => T = identity): List[T] =
    traverseUnsafe[T](maybeNextOfTwin): halfEdge =>
      f(halfEdge)

  def vertexTraversal[T](f: HalfEdge => T = identity): Either[TopologyError, List[T]] =
    traverse[T](maybeNextOfTwin): halfEdge =>
      f(halfEdge)
