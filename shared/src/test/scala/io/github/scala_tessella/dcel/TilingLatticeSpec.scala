...
class TilingLatticeSpec extends AnyFlatSpec with Matchers with OptionValues with TilingTestHelpers:
  ...
  it should "recover the lattice of a 6.6.6 hexagon net" in:
    val hexNet     = TilingBuilder.createHexagonNet(6, 6).value
    val lattice    = hexNet.translationLattice()
    lattice shouldBe defined
    // primitive cell of the honeycomb is one hexagon: area = 3*sqrt(3)/2 for unit edges
    val oneHexagon = 3.0 * math.sqrt(3.0) / 2.0
    covolume(lattice.value) shouldBe oneHexagon +- 1.0e-3

  it should "recover the same lattice on a larger 8x8 hexagon net" in:
    val hexNet  = TilingBuilder.createHexagonNet(8, 8).value
    val lattice = hexNet.translationLattice()
    lattice shouldBe defined
    covolume(lattice.value) shouldBe (3.0 * math.sqrt(3.0) / 2.0) +- 1.0e-3
  ...
  it should "recover the lattice despite stray triangles welded onto the boundary" in:
    ...
    // sanity: we actually added some triangles
  ...
  it should "cover a whole clean square net (every unit square is a complete cell)" in:
    ...
    block.corners.foreach(c => isVertexOf(c, net) shouldBe true)
  ...
  it should "return a block of whole cells inside a hexagon net" in:
    ...
    // past the complete-cell region, and snapping the parallelogram to the actual whole-face boundary is
    // build step 3 (run through parallelogonIndices).
    val net   = TilingBuilder.createHexagonNet(6, 6).value
    val block = net.largestContainedParallelogonBlock().value
    val covol = 3.0 * math.sqrt(3.0) / 2.0
    val total = net.innerFaces.size * covol
    block.area.toDouble should be > 0.0
    block.area.toDouble should be <= total + 1.0e-6
    (block.area / BigDecimal(covol)).toDouble shouldBe (block.cellsWide * block.cellsHigh).toDouble +- 1.0e-3
  ...
  it should "use the whole-boundary fast path for a single regular hexagon" in:
    val hexagon = TilingBuilder.createRegularPolygon(RegularPolygon(6))
    hexagon.largestContainedParallelogon().value.size shouldBe 6
  ...
  it should "give the same corners when foreign triangles are welded onto a square net" in:
    ...
    cleanCorners.foreach(c => grownCorners.exists(_.almostEquals(c)) shouldBe true)
