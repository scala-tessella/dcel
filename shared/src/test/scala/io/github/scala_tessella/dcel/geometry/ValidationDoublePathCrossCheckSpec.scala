...
/** ADR-0009 methodology step 4 — cross-check `SimplePolygon.fromUntrusted` (Double-path fast implementation,
  * candidate B) against the byte-for-byte preserved `BigDecimal`+Spire reference in
  ...
  */
class ValidationDoublePathCrossCheckSpec
    extends AnyFlatSpec
    with Matchers
