...
class TilingDeletionSpec extends AnyFlatSpec with Matchers with TilingTestHelpers:
  ...
  def disconnectingBoundaryFace: TilingDCEL =
    triangle
      .addRegularPolygonToBoundary(V1, RegularPolygon(4)).value
      .addRegularPolygonToBoundary(V4, RegularPolygon(3)).value
  ...
  it should "delete a face that would NOT partition the tiling in two parts" in:
    val s1s2     = square.addRegularPolygonToBoundary(V2, RegularPolygon(4)).value
    val s1s2s3   = s1s2.addRegularPolygonToBoundary(V2, RegularPolygon(4)).value
    val s1s2s3s4 = s1s2s3.addRegularPolygonToBoundary(V2, RegularPolygon(4)).value
    val result   = s1s2s3s4.deleteFace(F2)
    result.isRight shouldBe true

  it should "delete another face that would NOT partition the tiling in two parts" in:
    val s1s2   = hexagon.addRegularPolygonToBoundary(V2, RegularPolygon(6)).value
    val s1s2s3 = s1s2.addRegularPolygonToBoundary(V2, RegularPolygon(6)).value
  ...
  it should "successfully delete a boundary edge by deleting the adjacent face" in:
    val tiling = square
      .addRegularPolygonToBoundary(V2, RegularPolygon(4)).value // Two squares
    allAssert(
      tiling.innerFaces.length shouldBe 2, {
  ...
  it should "successfully delete a single inner edge, merging two faces" in:
    val tiling = square
      .addRegularPolygonToBoundary(V2, RegularPolygon(4)).value // Two squares sharing edge (V2, V3)
    allAssert(
      tiling.innerFaces.length shouldBe 2,
  ...
  it should "successfully delete multiple single inner edges" in:
    val tiling = triangle
      .addRegularPolygonToBoundary(V1, RegularPolygon(3)).value
      .addRegularPolygonToBoundary(V1, RegularPolygon(3)).value
      .addRegularPolygonToBoundary(V1, RegularPolygon(3)).value
      .addRegularPolygonToBoundary(V1, RegularPolygon(3)).value
      .addRegularPolygonToBoundary(V1, RegularPolygon(3)).value
