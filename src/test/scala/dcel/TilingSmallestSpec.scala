...
class TilingSmallestSpec extends AnyFlatSpec with Matchers with TilingTestHelpers:
  ...
  it should "add a triangle to a triangle, producing the smallest DCEL with a non boundary edge" in {
    ...
    tiling.outerFace.halfEdgesUnsafe.map(_.angle.get.toString).mkString(", ") shouldBe "240, 300, 240, 300"
    tiling.outerFace.halfEdgesUnsafe.map(_.incidentFace.get.id).mkString(", ") shouldBe "F0, F0, F0, F0"

    // Check inner face angles
    tiling.innerFaces.map(_.halfEdgesUnsafe.map(_.angle.get.toString).mkString(", ")) shouldBe List("60, 60, 60", "60, 60, 60")
