...
object TilingBuilder:
  ...
  def createHexagonNet(width: Int, height: Int, angle: AngleDegree = AngleDegree(120)): TilingDCEL =
    ...
    // Hexagon interior angle pattern: [alpha, beta, beta, alpha, beta, beta]
    // Sum must be 720 => 2*alpha + 4*beta = 720 => beta = 180 - alpha/2
    val alpha = angle
    val beta: AngleDegree = AngleDegree(180) - (alpha / 2)

    // Compute cumulative edge headings (unit edge length = 1).
    // Exterior turns are 180 - interior.
    ...
    // Edges 3,4,5 are negatives of 0,1,2 by construction.

    // Three global unit direction vectors g0,g1,g2 (others are the negatives).
    val g0x = spire.math.cos(h0.toBigDecimal);
    val g0y = spire.math.sin(h0.toBigDecimal) // usually (1,0)
    ...
    // Each hexagon is centrally symmetric with edges: +g0, +g1, +g2, -g0, -g1, -g2.
    // A single-orientation translational tiling is generated by lattice translations:
    //   T1 = g0 + g1,  T2 = g1 + g2.
    // We avoid floating deduplication by keying vertices with integer coefficients (n0,n1,n2)
    // such that position = n0*g0 + n1*g1 + n2*g2. This guarantees exact sharing across cells.

    // Corner offsets (relative to the start corner) expressed as integer triples in (g0,g1,g2):
    ...
    // For cell (i,j), base triple for the start corner (k=0) is:
    //   B = i*T1 + j*T2 = i*(g0+g1) + j*(g1+g2) = (i, i+j, j)
    ...
    // Helper: reusable twin pairs per directed vertex pair (store both directions)
    ...
    // Build all hex faces with correct unit-edges in directions g0,g1,g2
    for j <- 0 until height; i <- 0 until width do
      val face = faces(j)(i)
      ...
      // Interior angles per corner [alpha,beta,beta,alpha,beta,beta]
      val cornerAngles: Array[AngleDegree] =
        Array(alpha, beta, beta, alpha, beta, beta)

      // Directed half-edges CCW around the face
      val edgesCCW =
    ...
    // Boundary edges: those whose incidentFace is not defined yet (their twin belongs to a face)
    ...
    // Link and assign outer face, set boundary angles
