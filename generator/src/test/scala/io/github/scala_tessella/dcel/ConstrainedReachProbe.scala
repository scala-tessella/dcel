...
object ConstrainedReachProbe:
  ...
  def main(args: Array[String]): Unit =
    ...
    println(
      s"[constrained reach] n=$n maxFaces=$maxFaces perSet=${perSetMs / 60000}min — ${sets.size} type-sets"
    )
    for (ts, i) <- sets.zipWithIndex do
      ...
      println(
        f"  [${secs}%6.0fs] ${i + 1}%2d/${sets.size} ${label(ts)}%-46s reached=${onTarget.size}/$mult (+${(System.nanoTime() -
            b0) / 1e9}%.0fs)"
      )
    ...
    rows.sortBy(t => (t._1, t._2.toString)).foreach: (lbl, centres) =>
      val cs = centres.toList.sortBy((k, o) => (k, -o)).map((k, o) =>
        s"$k ${angle.getOrElse(o, s"$o?")}"
      ).mkString(", ")
